...
echo Installing/upgrading requirements.txt (GUI + launcher deps)...
python -m pip install --upgrade -r "requirements.txt" --quiet --progress-bar off
if errorlevel 1 (
...
if exist "dist\BandcampPlayer" (
    echo Removing previous dist folder...
    rmdir /s /q "dist\BandcampPlayer" 2>nul
)
for /f "tokens=1-3 delims=/: " %%a in ("%time%") do set TIMESTAMP=%%a:%%b:%%c
...
if exist "dist\BandcampPlayer\BandcampPlayer.exe" (
    for %%A in ("dist\BandcampPlayer\BandcampPlayer.exe") do (
        set FILE_SIZE=%%~zA
...
echo.
echo The launcher.exe is located in: dist\BandcampPlayer\BandcampPlayer.exe
echo.
pause
exit /b 0
